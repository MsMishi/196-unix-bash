#Chapter 1.6:
#Exercise 1: Write a command to print the string Use "man echo" instead.
$ echo 'Use "man echo"'

#Exercise 2: By running man sleep, figure out how to make the terminal "sleep" for 5 seconds, and execute the command to do so.
sleep 5

#Exercise 3: Execute the command to sleep for 5000 seconds, realize that's well over an hour, and then use the instructions from Box 4 to get out of trouble.
sleep 5000 ⌃C
